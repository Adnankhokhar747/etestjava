...
public class DebitInvoiceMapper {
    public static InvoiceDTO mapListToInvoiceDTO(DebitInvoice invoice){
        ...
        //set reamining here
        invoiceDTO.lineItems = invoice.getLineItemList().stream().map(DebitInvoiceMapper::mapToLineItemDTO).collect(Collectors.toList());
        return invoiceDTO;
    }

    public static LineItemDTO mapToLineItemDTO(DebitLineItem lineItem){
    ...
    public static InvoiceDTO mapToInvoiceDTO(DebitInvoice invoice) {
        List<DebitInvoice> invoiceList = new ArrayList<>();
        invoiceList.add(invoice);
        List<InvoiceDTO> collect = invoiceList.stream().map(DebitInvoiceMapper::mapListToInvoiceDTO).collect(Collectors.toList());
