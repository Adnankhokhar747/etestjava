...
public class DebitInvoiceB2CMapper {
    public static InvoiceDTO mapListToInvoiceDTO(DebitInvoiceB2C invoice){
        ...
        //set reamining here
        invoiceDTO.lineItems = invoice.getLineItemList().stream().map(DebitInvoiceB2CMapper::mapToLineItemDTO).collect(Collectors.toList());
        return invoiceDTO;
    }

    public static LineItemDTO mapToLineItemDTO(DebitB2CLineItem lineItem){
    ...
    public static InvoiceDTO mapToInvoiceDTO(DebitInvoiceB2C invoice) {
        List<DebitInvoiceB2C> invoiceList = new ArrayList<>();
        invoiceList.add(invoice);
        List<InvoiceDTO> collect = invoiceList.stream().map(DebitInvoiceB2CMapper::mapListToInvoiceDTO).collect(Collectors.toList());
